...
def load_csv_files(data_directory):
    """Load all CSV files from the ASSESS data directory"""
    csv_files = {}
...
def main():
    parser = argparse.ArgumentParser(description='Transform ALL ALS ASSESS CSV data using JSONata mapping')
    parser.add_argument('data_directory', help='Path to directory containing TREHALOSE CSV files')
    parser.add_argument('mapping_file', help='Path to JSONata mapping file (default: trehalose.jsonata)', 
                        default='mapping/trehalose.jsonata', nargs='?')
